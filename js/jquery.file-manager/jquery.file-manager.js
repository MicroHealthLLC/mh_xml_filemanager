...
*/
        ...
        _fn = {
            ...
            bindDoubleClick: function(explorer) {
                j('folder').dblclick(function(e) {
                    ...
                    j.ajax({
                        url: folder.data().path + '/root.json'
                ...
                j('file').dblclick(function(e) {
                    var file = j(this);
                //    if (file.data().path.indexOf('.xml') > -1) {
                    if ((file.data().path.toLowerCase().indexOf('.xml') > -1) || (file.data().path.toLowerCase().indexOf('.exp') > -1)) {
                        window.location = "xml?file=" + file.data().path;
                     } else {
                        var fileData = file.data(),
                            win = window.open(fileData.path);
                        var setWindowTitle1 = function () {
                            if (win.document) {
                                win.document.title = fileData.name;
                            } else {
                                setTimeout(setWindowTitle1, 10);
                            }
                        };
                        setWindowTitle1();
                    }
            ...
            renderExplorer: function(explorer, filesArray) {
                explorer.html('<div class="bg" style="margin-top:0px;position:absolute;top:0;left:0;right:0;bottom:0;z-index:0;"></div>');
    ...
    j.fn.extend({
        fileManager: function(jsonData) {
         ...
         //NOTE: Added trim() and toLowerCase(), but this search feature still has bugs!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    file_manager.viewport.delegate('.fileSearchBox', 'keyup', function() {
                        var key = j(this).val().trim(),
